...
class AppleStrategy extends passport.Strategy {
            ...
            try {
                this._stateStore.verify(req, state, (err, ok, state) => {
                    ...
                    oauth2.getOAuthAccessToken(code, params, (err, accessToken, refreshToken, params) => {
                        ...
                        this.extractProfileFromToken(idToken, req.body.user, req, null, (err, profile) => {
                            if (err) return this.error(err);
    ...
    extractProfileFromToken(idToken, user, clientID, req, cb) {
        const verifyOpts = {
            audience: clientID ? clientID : this._clientID,
